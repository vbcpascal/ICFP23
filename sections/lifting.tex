...
\begin{proof}
  ...
  The first equation is based on requirement \ref{req:pat}.
...
\begin{theorem}[Strong Abstraction]
  ...
  the derived rule maintain abstraction.
\end{theorem}
...
\begin{proof}
  ...
  Due to the finite depth of RHS of the translation rule,
   ...
   or to the evaluation of constant expressions.
  Because all the rules in expansion are structural, no other constructors except value constructors are introduced.
\end{proof}
...
\end{aligned} \right. \]
This rule can be described in our meta-language:
\[ (λx:t.~e_1)[e_2/y] \cqq ():\branch{
    \_ \mid x=y |> λx:t.~e_1 \\&
    \_ |> λx:t.~e_1[e_2/y]} \]

We would like to define an operation $\omega$ to describe the substituion behaviour of an expression $\mexp$,
 whose output is a bone.
