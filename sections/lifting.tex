...
In this section, we will discuss translation rules and semantics lifting algorithm.
...
We first give some concepts of translation rules and impose some requirements.
...
After that, nonstructural parts, lambda abstraction and application will be discussed.
Finally, we will provide some extensions to refine the algorithm and proofs.
...
% Note that the RHS of the translation rule satisfies \ref{ll:let}, and no language constructs will be generated in lambda exposure.
...
\subsection{Preprocessing: Lambda Exposure}
...
Fortunately, lambda exposure can help us.
...
\begin{align}
  ...
  \laml{\mexp_b} & = \mexp_b
\end{align}
where $\laml{\bullet}$ is lambda exposure transformation, with the generation of some translation rules.
...
Note that the newly generated $c_n'$ needs to process lambda exposure recursively,
 and may generate some more translation rules.
The lambda exposure is pretty like the lambda lifting for inner abstractions \cite{lambda-lifting}.

% The following lemma clearly holds.

\begin{lemma}
  Lambda exposure preserves requirements \ref{req:no-recursion} and \ref{req:close}.
\end{lemma}

The correctness of lambda exposure is pretty obvious.
...
\begin{theorem}
  Lambda exposure maintains correctness and abstraction.
\end{theorem}

In addition, Lambda exposure is applicable to any language construct that produces delayed evaluations.
