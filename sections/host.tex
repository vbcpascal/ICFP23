...
For example, the following rule defines the semantic of references (Types and Programming Languages \cite{tapl}, chap. 13).
...
Furthermore, we introduce substitution in the meta-language.
Substitution is a special metalanguage operation, but not a hook or filter. 
% \footnote{But it can be defined as a filter in Skeleton.}
...
% We will illustrate the semantics of the above rules in two ways. 
% First we give the translation of the above rule to Haskell as our method to implement the interpreter for language defined.
% Then we show its operational semantics, which will be used in the following proofs. 
% We will not elaborate on the consistency of the two.
...
%    $\mexp_{x_i}$ is not used to be matched, as an argument in any filter, applied by any constructor, or as the result value in $b$;
  % \item all the expressions that are hooked $H(\mexp)$ in $b$ must be $\mexp_{x_i}$.
  % \item all the constructor in $b$ must used to construct a value.
% \end{itemize}
\end{definition}
